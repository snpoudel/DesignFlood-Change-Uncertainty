...
stn = pd.read_csv(f'data/num_gauge_precip_basinwise_2010-2020/basin_{basin_id}.csv')
...
def func_multiprocessing(grid_input):
        ...
        stn_always_keep_index = 0 #always keep this index in the combination
        #check the list stn_keep_index and if doesn't have stn_always_keep_index, then add it by replacing the first element
        for i in range(len(stn_keep_index)):
            if stn_always_keep_index not in stn_keep_index[i]:
                stn_keep_index[i][0] = stn_always_keep_index
        ...
        for comb in range(10):
            ...
            precip_all_stations = pd.read_csv('data/preprocessed_all_stations_precip_2010-2020.csv')
            ...
            precip_df.to_csv(f'data/idw_precip_inside01/idw_precip{basin_id}_coverage{grid_input}_comb{comb}.csv', index=False)

if __name__ == '__main__':
    pool = multiprocessing.Pool(processes=6)
    grid_input = np.array([0.05, 0.15, 0.2, 0.3, 0.4, 0.8])
