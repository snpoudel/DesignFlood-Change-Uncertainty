...
for method in ['mle', 'lm']:
    for distribution in ['gum', 'gev']:
        ...
        for basin_id in used_basin_list:
            ...
            flood_5, flood_10, flood_20 = return_flood(data,5,distribution,method), return_flood(data,10,distribution,method), return_flood(data,20,distribution,method)
            ...
            hbv_true_future = pd.read_csv(f'output/future/hbv_true_future_streamflow/hbv_true_future_output_{basin_id}.csv')
            ...
            for grid in grid_list:
                for comb in range(15):
                    ...
                    if os.path.exists(file_path):
                        ...
                        hbv_true = pd.read_csv(f'output/hbv_idw_streamflow/hbv_idw_streamflow{basin_id}_coverage{grid}_comb{comb}.csv')
                        ...
                        #Hymod model
                        #read the streamflow data
                        hymod = pd.read_csv(f'output/simp-hymod/hymod_interpol_streamflow{basin_id}_coverage{grid}_comb{comb}.csv')
                        hymod['year'] = pd.to_datetime(hymod['date']).dt.year
                        data = hymod.groupby('year')['streamflow'].max()
                        #calculate the 5, 10, and 20 years flood
                        flood_5, flood_10, flood_20 = return_flood(data,5,distribution,method), return_flood(data,10,distribution,method), return_flood(data,20,distribution,method)
                        ...
                        hbv_true_future = pd.read_csv(f'output/future/hbv_idw_future_streamflow/hbv_idw_future_streamflow{basin_id}_coverage{grid}_comb{comb}.csv')
                        hbv_true_future['year'] = pd.to_datetime(hbv_true_future['date']).dt.year
                        data = hbv_true_future.groupby('year')['streamflow'].max()
                        #calculate the 5, 10, and 20 years flood
                        flood_5, flood_10, flood_20 = return_flood(data,5,distribution,method), return_flood(data,10,distribution,method), return_flood(data,20,distribution,method)
                        ...
                        #read the streamflow data
                        hbv_recalibrated_future =pd.read_csv(f'output/future/hbv_idw_recalib_future_streamflow/hbv_idw_recalib_future_streamflow{basin_id}_coverage{grid}_comb{comb}.csv')
                        ...
                        #read the streamflow data
                        hymod_future = pd.read_csv(f'output/future/hymod_idw_future_streamflow/hymod_interpol_future_streamflow{basin_id}_coverage{grid}_comb{comb}.csv')
                        ...
                        #Hymod model future
                        #read the streamflow data
                        hymod_future = pd.read_csv(f'output/future/simp-hymod/hymod_interpol_future_streamflow{basin_id}_coverage{grid}_comb{comb}.csv')
                        hymod_future['year'] = pd.to_datetime(hymod_future['date']).dt.year
                        data = hymod_future.groupby('year')['streamflow'].max()
                        #calculate the 5, 10, and 20 years flood
                        flood_5, flood_10, flood_20 = return_flood(data,5,distribution,method), return_flood(data,10,distribution,method), return_flood(data,20,distribution,method)
                        ...
                        change_hbv_recalib = pd.DataFrame({'station':basin_id,'model':'HBV Recalib',
                                        'change_5yr_flood':true_change(temp_df_hbvrf['5yr_flood'] , temp_df_hbvr['5yr_flood'], true_change_5yr ),
                                        'change_10yr_flood':true_change(temp_df_hbvrf['10yr_flood'] , temp_df_hbvr['10yr_flood'], true_change_10yr),
                                        'change_20yr_flood':true_change(temp_df_hbvrf['20yr_flood'] , temp_df_hbvr['20yr_flood'], true_change_20yr), 'precip_rmse':precip_rmse}, index=[0])
                        
                        change_full_hymod = pd.DataFrame({'station':basin_id,'model':'Full-Hymod', 
                                        'change_5yr_flood':true_change(temp_df_fullhyf['5yr_flood'] , temp_df_fullhy['5yr_flood'], true_change_5yr),
                                        'change_10yr_flood':true_change(temp_df_fullhyf['10yr_flood'] , temp_df_fullhy['10yr_flood'], true_change_10yr),
                                        'change_20yr_flood':true_change(temp_df_fullhyf['20yr_flood'] , temp_df_fullhy['20yr_flood'], true_change_20yr), 'precip_rmse':precip_rmse}, index=[0])
                        
                        change_lstm = pd.DataFrame({'station':basin_id,'model':'LSTM', 
                                        'change_5yr_flood':true_change(temp_df_lstmf['5yr_flood'] , temp_df_lstm['5yr_flood'], true_change_5yr),
                                        'change_10yr_flood':true_change(temp_df_lstmf['10yr_flood'] , temp_df_lstm['10yr_flood'], true_change_10yr),
                                        'change_20yr_flood':true_change(temp_df_lstmf['20yr_flood'] , temp_df_lstm['20yr_flood'], true_change_20yr), 'precip_rmse':precip_rmse}, index=[0])
                        
                        change_full_hymod_lstm = pd.DataFrame({'station':basin_id,'model':'FULL-HYMOD-LSTM', 
                                        'change_5yr_flood':true_change(temp_df_lstm_fullhymodf['5yr_flood'] , temp_df_lstm_fullhymod['5yr_flood'], true_change_5yr),
                                        'change_10yr_flood':true_change(temp_df_lstm_fullhymodf['10yr_flood'] , temp_df_lstm_fullhymod['10yr_flood'], true_change_10yr),
                                        'change_20yr_flood':true_change(temp_df_lstm_fullhymodf['20yr_flood'] , temp_df_lstm_fullhymod['20yr_flood'], true_change_20yr), 'precip_rmse':precip_rmse}, index=[0])
                        
                        change_hymod_lstm = pd.DataFrame({'station':basin_id,'model':'HYMOD-LSTM', 
                                        'change_5yr_flood':true_change(temp_df_lstm_hymodf['5yr_flood'] , temp_df_lstm_hymod['5yr_flood'], true_change_5yr),
                                        'change_10yr_flood':true_change(temp_df_lstm_hymodf['10yr_flood'] , temp_df_lstm_hymod['10yr_flood'], true_change_10yr),
                                        'change_20yr_flood':true_change(temp_df_lstm_hymodf['20yr_flood'] , temp_df_lstm_hymod['20yr_flood'], true_change_20yr), 'precip_rmse':precip_rmse}, index=[0])
                        
                        change_hymod = pd.DataFrame({'station':basin_id,'model':'Hymod', 
                                        'change_5yr_flood':true_change(temp_df_hyf['5yr_flood'] , temp_df_hy['5yr_flood'], true_change_5yr),
                                        'change_10yr_flood':true_change(temp_df_hyf['10yr_flood'] , temp_df_hy['10yr_flood'], true_change_10yr),
                                        'change_20yr_flood':true_change(temp_df_hyf['20yr_flood'] , temp_df_hy['20yr_flood'], true_change_20yr), 'precip_rmse':precip_rmse}, index=[0])
...
df_change.to_csv(f'output/allbasins_change_tyr_flood_value.csv', index=False)