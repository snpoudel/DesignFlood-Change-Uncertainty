...
simp_hymod_params = pd.DataFrame()
grid = 99
for comb in np.arange(10):
    if os.path.exists(f'output/parameters/simp_hymod/params{id}_grid{grid}_comb{comb}.csv'):
        file = pd.read_csv(f'output/parameters/simp_hymod/params{id}_grid{grid}_comb{comb}.csv')
        file['tag'] = f'error0'
        simp_hymod_params=pd.concat([simp_hymod_params, file], ignore_index=True)
#merge truth
df_error0 = simp_hymod_params
...
for comb in np.arange(10):
    if os.path.exists(f'output/parameters/simp_hymod/params{id}_grid{grid}_comb{comb}.csv'):
        ...
        simp_hymod_params=pd.concat([simp_hymod_params, file], ignore_index=True)
#merge truth
df_error1out = simp_hymod_params
...
fig.savefig(f'output/figures/01108000/simp_hymod_calib_parameters.png', dpi=300)