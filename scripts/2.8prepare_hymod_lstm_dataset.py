...
#     for case in ['']: #iterate over historical and future case
...
#         previous_file.to_csv(f'data/regional_lstm_hymod/{case}processed_lstm_train_datasets/pb0/lstm_input{id}.csv', index=False)
...
#                         #save the final lstm input file
#                         previous_file.to_csv(f'data/regional_lstm_hymod/processed_lstm_train_datasets/pb{precip_bucket}/lstm_input{id}.csv', index=False)
...
for id in basin_list['basin_id']:
    for case in ['', 'future_']: #iterate over historical and future case
        ...
        previous_file['era5temp'] = true_flow['era5temp']
        if case == 'future_':
            temp_future = pd.read_csv(f'data/future/future_temperature/future_temp{id}.csv')
            previous_file['era5temp'] = temp_future['tavg']
        # previous_file['hymod_flow'] = hymod_flow['streamflow']
        previous_file['date'] = true_flow['date']
        previous_file['qobs_error'] = true_flow['streamflow'] - hymod_flow['streamflow']
        previous_file = previous_file.reset_index(drop=True)
        ...
        if case == '':
            ...
            tag = 'future'
        previous_file.to_csv(f'data/regional_lstm_hymod/processed_lstm_prediction_datasets/{tag}/pb0/lstm_input{id}_coverage99_comb0.csv', index=False)
...
#                     #save the final lstm input file
#                     previous_file.to_csv(f'data/regional_lstm_hymod/processed_lstm_prediction_datasets/historical/pb{precip_bucket}/lstm_input{id}_coverage{coverage}_comb{comb}.csv', index=False)
        ...
        for coverage in range(15):
            for comb in range(15):
                ...
                if os.path.exists(file_path):
                    ...
                    #read true discharge for this basin
                    true_flow = pd.read_csv(f'output/hbv_true/hbv_true{id}.csv')
                    future_temp = pd.read_csv(f'data/future/future_temperature/future_temp{id}.csv')
                    #read hymod discharge
                    hymod_flow = pd.read_csv(f'output/future/hymod/hymod{id}_coverage{coverage}_comb{comb}.csv')

                    #merge data,temp,flow from true file to previous file
                    previous_file['noisy_precip'] = noisy_precip['PRECIP']
                    previous_file['era5temp'] = future_temp['tavg']
                    # previous_file['hymod_flow'] = hymod_flow['streamflow']
                    previous_file['date'] = true_flow['date']
                    previous_file['qobs_error'] = true_flow['streamflow'] - hymod_flow['streamflow']
                    previous_file = previous_file.reset_index(drop=True)
                    ...
                    #save the final lstm input file
                    previous_file.to_csv(f'data/regional_lstm_hymod/processed_lstm_prediction_datasets/future/pb{precip_bucket}/lstm_input{id}_coverage{coverage}_comb{comb}.csv', index=False)