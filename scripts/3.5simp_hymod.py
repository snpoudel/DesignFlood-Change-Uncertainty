...
for combination in np.arange(12):
    ...
    if os.path.exists(file_path):
        ...
        output_df.to_csv(f'output/simp_hymod/simp_hymod{id}_coverage{grid}_comb{combination}.csv')
        ...
        output_df = pd.DataFrame({ 'date':precip_in['DATE'], 'streamflow':q_sim })
        #save output dataframe
        output_df.to_csv(f'output/future/simp_hymod/simp_hymod{id}_coverage{grid}_comb{combination}.csv')
