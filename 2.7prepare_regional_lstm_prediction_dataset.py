...
for id in basin_list['basin_id']:
    for case in ['', 'future_']: #iterate over historical and future case
        ...
        if case == '':
            ...
            tag = 'future'
        previous_file.to_csv(f'data/regional_lstm/prediction_datasets/{tag}/pb0/lstm_input{id}_coverage99_comb0.csv', index=False)
        ...
        for coverage in range(15):
            for comb in range(15):
                ...
                if os.path.exists(file_path):
                    ...
                    #save the final lstm input file
                    previous_file.to_csv(f'data/regional_lstm/prediction_datasets/historical/pb{precip_bucket}/lstm_input{id}_coverage{coverage}_comb{comb}.csv', index=False)
        ...
        for coverage in range(15):
            for comb in range(15):
                ...
                if os.path.exists(file_path):
                    ...
                    #save the final lstm input file
                    previous_file.to_csv(f'data/regional_lstm/prediction_datasets/future/pb{precip_bucket}/lstm_input{id}_coverage{coverage}_comb{comb}.csv', index=False)