...
def calibNSE(station_id, grid, combination):
    ...
    def nse(pars):
        ...
        q_obs = q_obs[730:] #remove first 2 years
        #calculate nse
...
for combination in np.arange(12):
    ...
    if os.path.exists(file_path):
        ...
        output_df.to_csv(f'output/rehbv/rehbv{id}_coverage{grid}_comb{combination}.csv')
        ...
        output_df = pd.DataFrame({ 'date':precip_in['DATE'], 'streamflow':q_sim })
        #save output dataframe
        output_df.to_csv(f'output/future/rehbv/rehbv{id}_coverage{grid}_comb{combination}.csv')