...
for combination in np.arange(12):
    ...
    if os.path.exists(file_path):
        ...
        #save parameters
        params_in.to_csv(f'output/parameters/simp-hymod/params{id}_grid{grid}_comb{combination}.csv')
        ...
        output_df.to_csv(f'output/simp-hymod/hymod_interpol_streamflow{id}_coverage{grid}_comb{combination}.csv')
        ...
        output_df = pd.DataFrame({ 'date':precip_in['DATE'], 'streamflow':q_sim })
        #save output dataframe
        output_df.to_csv(f'output/future/simp-hymod/hymod_interpol_future_streamflow{id}_coverage{grid}_comb{combination}.csv')
