    ...
    for coverage in range(105):
        for comb in range(15):
            ...
            if os.path.exists(file_path):
                ...
                hymod_simflow = pd.read_csv(f'output/hymod_idw_streamflow/hymod_interpol_streamflow{id}_coverage{coverage}_comb{comb}.csv')
                #merge by date
...
for id in basin_list:
    for coverage in range(105):
        for comb in range(15):
            ...
            if os.path.exists(file_path):
                lstm_error = pd.read_csv(file_path)
                hymod_simflow = pd.read_csv(f'output/future/hymod_idw_future_streamflow/hymod_interpol_future_streamflow{id}_coverage{coverage}_comb{comb}.csv')
                #merge by date
                temp_df = pd.merge(lstm_error, hymod_simflow, on='date', how='inner')
                temp_df['hymod_lstm_streamflow']=temp_df['streamflow']+temp_df['streamflow_error']
