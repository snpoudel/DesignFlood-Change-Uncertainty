...
for id in used_basin_list:
    ...
    plt.suptitle('Diagnostics plot for validation period')
    sns.scatterplot(data=nse_melt, x='RMSE(PRECIP)', y='value', hue='variable',size=None,sizes=(3,3), alpha=0.6, ax=axs[0])
    ...
    for variable in nse_melt['variable'].unique():
        ...
        axs[0].plot(lowess[:, 0], lowess[:, 1], label=None, linewidth=1)
    axs[0].set_xlabel('Precipitation RMSE (mm/day)')
    axs[0].set_ylabel('Streamflow NSE')
    axs[0].grid(True, linestyle ='--', alpha = 0.5)
    axs[0].legend(title='', loc='best')
    new_labels = ['HBV/Truth', 'RECALIBRATED HBV', 'HYMOD', 'LSTM']
    for t, l in zip(axs[0].get_legend().texts, new_labels):t.set_text(l)
    sns.scatterplot(data=kge_melt, x='RMSE(PRECIP)', y='value', hue='variable',size=None,sizes=(3,3), alpha=0.6,ax=axs[1])
    # Compute and plot LOWESS lines
    for variable in kge_melt['variable'].unique():
        subset = kge_melt[kge_melt['variable'] == variable]
        lowess = sm.nonparametric.lowess(subset['value'], subset['RMSE(PRECIP)'], frac=0.5)  # Adjust frac as needed
        axs[1].plot(lowess[:, 0], lowess[:, 1], label=None, linewidth=1)
    # Customize the plot
    ...
    sns.scatterplot(data=rmse_melt, x='RMSE(PRECIP)', y='value', hue='variable',size=None,sizes=(3,3), alpha=0.6,ax=axs[2])
    # Compute and plot LOWESS lines
    for variable in rmse_melt['variable'].unique():
        subset = rmse_melt[rmse_melt['variable'] == variable]
        lowess = sm.nonparametric.lowess(subset['value'], subset['RMSE(PRECIP)'], frac=0.5)  # Adjust frac as needed
        axs[2].plot(lowess[:, 0], lowess[:, 1], label=None, linewidth=1)
    # Customize the plot
    ...
    plt.suptitle('Diagnostics plot for validation period')
    #Plot scatter points
    sns.scatterplot(data=bias_melt, x='RMSE(PRECIP)', y='value', hue='variable',size=None,sizes=(3,3), alpha=0.5, ax=axs[0])
    # Compute and plot LOWESS lines
    ...
    new_labels = ['HBV/Truth', 'RECALIBRATED HBV', 'HYMOD', 'LSTM']
    for t, l in zip(axs[0].get_legend().texts, new_labels):t.set_text(l)
    sns.scatterplot(data=hfb_melt, x='RMSE(PRECIP)', y='value', hue='variable',size=None,sizes=(3,3), alpha=0.6, ax=axs[1])
    # Compute and plot LOWESS lines
