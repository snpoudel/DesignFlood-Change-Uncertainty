...
for method in ['mle']: #['mle', 'lm']:
    for distribution in ['gev']:
        ...
        for basin_id in used_basin_list:
            ...
            true_precip = pd.read_csv(f'Z:/MA-Precip-Uncertainty-GaugeData/data/true_precip/true_precip{basin_id}.csv')
            future_true_precip = pd.read_csv(f'Z:/MA-Precip-Uncertainty-GaugeData/data/future/future_true_precip/future_true_precip{basin_id}.csv')

            #HBV truth model
            #read the streamflow data
            hbv_truth = pd.read_csv(f'Z:/MA-Precip-Uncertainty-GaugeData/output/hbv_true/hbv_true{basin_id}.csv')
            ...
            hbv_true_future = pd.read_csv(f'Z:/MA-Precip-Uncertainty-GaugeData/output/future/hbv_true/hbv_true{basin_id}.csv')
            ...
            for grid in grid_list:
                for comb in range(15):
                    ...
                    if os.path.exists(file_path):
                        idw_precip = pd.read_csv(f'Z:/MA-Precip-Uncertainty-GaugeData/data/noisy_precip/noisy_precip{basin_id}_coverage{grid}_comb{comb}.csv')
                        future_idw_precip = pd.read_csv(f'Z:/MA-Precip-Uncertainty-GaugeData/data/future/future_noisy_precip/future_noisy_precip{basin_id}_coverage{grid}_comb{comb}.csv')
                        ...
                        df_change_flood = pd.concat([df_change_flood, change_hbv_recalib,  change_full_hymod, change_hymod], ignore_index=True)
