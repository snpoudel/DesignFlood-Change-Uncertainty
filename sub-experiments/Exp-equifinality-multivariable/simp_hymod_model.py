...
def hymod(pars, p, temp, date, latitude, routing):
    ...
    hmax = pars[0] # maximum storage/indicator height
    ...
    if pars[1] == 2: #to avoid undefined in log calculation
        bexp = 10**6 #a high value to represent infinity
    else:
        bexp = np.log(1 - pars[1] / 2) / np.log(0.5)    
    alpha = pars[2] #parameter which divides runoff into quick/direct flow and slow/base flow
    ks = pars[3] #resident time in slow release reservoir/ base flow reservoir
    lmax = pars[4] #maximum groundwater storage

    coeff_pet = pars[5] #coefficient for potential evapotranspiration

    ddf = pars[6] #degree-day factor
    #scf = pars[9] #snowfall correction factor
    #ts = pars[10] #threshold temperature for snow falling
    tm = pars[7] #threshold temperature for snowmelt
    ...
    maxbas = pars[8] #traingular weighting function routing parameter, it represents time taken by water to move through the catchment and reach outlet
    ...
    for t in range(1, len(p)):
        ...
        if ct >= (tm): #All rain, no snow
            snow = 0
            rain = cp
        else:  # All snow, no rain
            snow = cp
            rain = 0
        ...
        if state_snow > 0:
            ...
            liqmax = state_snow * 0.1 #water holding capacity of snow usually taken as 0.1
    ...
    return qtotal, actevap, state_soil 
#End of function  