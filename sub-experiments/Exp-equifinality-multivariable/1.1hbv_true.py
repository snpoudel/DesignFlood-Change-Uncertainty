...
for id in basin_list['basin_id']:
    ...
    output_df = pd.DataFrame({'date':precip_in['DATE'], 'true_precip':precip_in['PRECIP'],'era5temp':temp_in['tavg'],
                            'latitude':lat_in, 'streamflow':q_sim, 'evapotranspiration':et_sim, 'soilmoisture':sm_sim })
    #save output dataframe
    output_df.to_csv(f'Z:/MA-Precip-Uncertainty-Exp-equifinality/output/hbv_true/hbv_true{id}.csv', index=False)
    ...
    output_df = pd.DataFrame({'date':precip_in['DATE'], 'true_precip':precip_in['PRECIP'],'era5temp':temp_in['tavg'],
                            'latitude':lat_in, 'streamflow':q_sim, 'evapotranspiration':et_sim, 'soilmoisture':sm_sim })
    #save output dataframe
    output_df.to_csv(f'Z:/MA-Precip-Uncertainty-Exp-equifinality/output/future/hbv_true/hbv_true{id}.csv', index=False)