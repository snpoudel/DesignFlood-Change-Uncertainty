...
def calibNSE(station_id, grid, combination):
    ...
    def multiobj_nse(pars):
        q_sim, et_sim, sm_sim = hbv(pars, p, temp, date, latitude, routing)
...
for combination in np.arange(12):
    ...
    if os.path.exists(file_path):
        ...
        # params_in.to_csv(f'/home/fs01/sp2596/MA-Precip-Uncertainty-Exp-equifinality/output/parameters/hbv_recalib/params{id}_grid{grid}_comb{combination}.csv')

        params_in = pd.read_csv(f'/home/fs01/sp2596/MA-Precip-Uncertainty-Exp-equifinality/output/parameters/hbv_recalib/params{id}_grid{grid}_comb{combination}.csv')
        ...
        # #run hbv model
        # q_sim, et_sim, sm_sim = hbv(params_in, precip_in['PRECIP'], temp_in['tavg'], precip_in['DATE'], lat_in, routing=1)
        ...
        # output_df.to_csv(f'/home/fs01/sp2596/MA-Precip-Uncertainty-Exp-equifinality/output/rehbv/rehbv{id}_coverage{grid}_comb{combination}.csv')
        ...
        output_df = pd.DataFrame({ 'date':precip_in['DATE'], 'streamflow':q_sim, 'evapotranspiration':et_sim, 'soilmoisture':sm_sim })
        #save output dataframe
        output_df.to_csv(f'/home/fs01/sp2596/MA-Precip-Uncertainty-Exp-equifinality/output/future/rehbv/rehbv{id}_coverage{grid}_comb{combination}.csv')