        ...
        for coverage in range(101):
            for combination in range(15):
                ...
                if os.path.exists(file_path):
                    ...
                    gr4j_params = pd.read_csv(f'output/parameters/regr4j/params{id}_grid{coverage}_comb{combination}.csv', dtype = {'station_id':str})
                    # gr4j_params = gr4j_params.iloc[:,:-2] #remove undesired columns

                    params_in = gr4j_params.drop(columns=['Unnamed: 0', 'station_id', 'nse'])
                    ...
                    q_sim = np.round(np.array(q_sim), 4)
                    output_df = pd.DataFrame({ 'date':precip_in['DATE'], 'streamflow':q_sim })
                    #save output dataframe
                    output_df.to_csv(f'output/{scenario}/regr4j/regr4j{id}_coverage{coverage}_comb{combination}.csv', index=False)
