...
def calibNSE(station_id, grid, combination):
    ...
    algorithm_param = {
        ...
        'population_size': 1000, #1000                # Number of parameter-sets in a single iteration/generation(to start with population 10 times the number of parameters should be fine!)
...
for combination in np.arange(12):
    ...
    if os.path.exists(file_path):
        ...
        #save parameters
        params_in.to_csv(f'output/parameters/simp_hymod/params{id}_grid{grid}_comb{combination}.csv')

        # params_in = pd.read_csv(f'output/parameters/simp_hymod/params{id}_grid{grid}_comb{combination}.csv')
        ...
        output_df.to_csv(f'output/baseline/simp_hymod/simp_hymod{id}_coverage{grid}_comb{combination}.csv', index=False)